...
from analyst1_consts import (
    ANALYST1_CONNECTIVITY_ENDPOINT,
    ANALYST1_ERR_CONNECTIVITY_TEST,
    ANALYST1_SUCC_CONNECTIVITY_TEST,
)


class Analyst1Connector(BaseConnector):
    """
    Analyst1 connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = ANALYST1_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(ANALYST1_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(ANALYST1_SUCC_CONNECTIVITY_TEST)
